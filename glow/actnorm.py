...
class ActNorm(nn.Module):
    ...
    def apply_bias(self, x, logdet, reverse):
        if not reverse:
            x = x + self.b
            assert not np.isnan(x.mean().item()), "nan after apply_bias in forward: x=%0.3f, b=%0.3f" % (
                x.mean().item(), self.b.mean().item())
            assert not np.isinf(x.mean().item()), "inf after apply_bias in forward: x=%0.3f, b=%0.3f" % (
                x.mean().item(), self.b.mean().item())
            return x, logdet
        if reverse:
            x = x - self.b
    ...
    def apply_scale(self, x, logdet, reverse):
        if not reverse:
            ...
            assert not np.isnan(x.mean().item()), "nan after apply_scale in forward: x=%0.3f, logs=%0.3f" % (
                x.mean().item(), self.logs.mean().item())
            assert not np.isinf(x.mean().item()), "inf after apply_scale in forward: x=%0.3f, logs=%0.3f" % (
                x.mean().item(), self.logs.mean().item())
            assert not np.isnan(
                logdet.sum().item()), "nan in log after apply_scale in forward: logdet=%0.3f, logs=%0.3f" % (
                logdet.mean().item(), self.logs.mean().item())
            assert not np.isinf(
                logdet.sum().item()), "inf in log after apply_scale in forward: logdet=%0.3f, logs=%0.3f" % (
                logdet.mean().item(), self.logs.mean().item())
            return x, logdet
        if reverse:
    ...
    def forward(self, x, logdet=None, reverse=False):
        ...
        if not reverse:
            ...
            loss_std = ((x - loss_mean) ** 2).mean(dim=0, keepdim=True).mean(dim=2, keepdim=True).mean(dim=3,
                                                                                                       keepdim=True).mean()
...
if __name__ == "__main__":
    ...
    for i in range(5000):
        ...
        opt.step()
        if i % 10 == 0:
            print("\r loss=%0.3f | logdet=%0.3f | b=%0.3f | s=%0.3f | rev=%0.3f " % (
                loss, -logdet2.mean().item(), actnorm.b.mean().item(), np.exp(actnorm.logs.mean().item()), rev_loss),
                end="\r")
