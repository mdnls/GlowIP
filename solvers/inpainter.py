    ...
    for gamma in loopOver:
        ...
        if args.save_results:
            ...
            if args.init_strategy == 'random':
                ...
                save_path = save_path % (
                    args.inpaint_method, args.mask_size, gamma, args.steps, args.lr, args.init_std, args.optim)
            else:
                save_path = save_path + "/inpaint_%s_masksize_%0.4f_gamma_%0.6f_steps_%d_lr_%0.3f_init_%s_optim_%s"
                save_path = save_path % (
                    args.inpaint_method, args.mask_size, gamma, args.steps, args.lr, args.init_strategy, args.optim)
...
def gen_mask(maskType, imgSize, masksize=0.25):
    ...
    if maskType == 'random':
        ...
        mask = np.ones(image_shape)
        low = int(imgSize * center_scale)
        up = int(imgSize * (1.0 - center_scale))
        mask[low:up, low:up] = 0.0
    elif maskType == 'left':
        mask = np.ones(image_shape)
        c = imgSize  # // 2
        masksize = 1 - masksize
